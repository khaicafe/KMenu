...
async function menu() {
    const permissionManager: any = await checkroles('GET:manager/dashboard')
    console.log('test permission', permissionManager)
    let menu = [] as any
    menu = [...mainItem]
    // manage
    permissionManager ? menu.push(...mainItemManager): null
    // check product
    import.meta.env.PROD ? null : menu.push(...templateItem)
   
    return menu
}
const sidebarItem: menu[] = await menu()
